...
# File: HashtagsAnalysis.R
# Objective: Analysis of hashtags
# Researcher: rll307
# Date: 10/02/22 - 22:39:07
...
#Step 3 ----------
#Selecting the hash
d1.hash.dfm <- dfm_select(d1.dfm, pattern = ("#*"))

#Step 4 ----------
#Selecting the most common hash
D1.top.hash <- names(topfeatures(d1.hash.dfm , 100))

#Step 5 ----------
# Creating a user's FCM
d1.hash.fcm <- fcm(d1.hash.dfm)

#Slecting the most relevant hash from the total FCM
d1.hash.top <- fcm_select(d1.hash.fcm, pattern = D1.top.hash)
...
d1.hash.top %>%
  ...
  write.csv(., 'hash01.csv',row.names= FALSE)